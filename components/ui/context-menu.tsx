    ...
    <div className="w-full">
      <div className="relative rounded-xl overflow-hidden" style={{ aspectRatio: '3/4', background: 'rgba(0,0,0,0.2)' }}>
        ...
        <button
          ...
          className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 backdrop-blur-sm text-white flex items-center justify-center text-sm hover:bg-black/60 transition-all disabled:opacity-30"
        >
          ‹
        </button>
        <button
          onClick={() => flip('next')}
          className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 backdrop-blur-sm text-white flex items-center justify-center text-sm hover:bg-black/60 transition-all"
        >
          ›
        </button>
        {/* Page indicator */}
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-black/40 backdrop-blur-sm text-white text-xs">
          صفحة {currentPage}
        </div>
      </div>
  ...
  return (
    ...
    >
      {theme.backgroundType === 'animated' && theme.animatedBackground && (
        <AnimatedBackground
          type={theme.animatedBackground}
          color={theme.accentColor}
          speed={theme.animatedBackgroundSpeed ?? 1}
        />
      )}
      ...
      <div className="relative z-10 min-h-screen w-full flex flex-col items-center px-4 py-8 sm:py-12 md:py-16">

        {/* Avatar */}
        {!profile.hideAvatar && (
          <div className={cn('mb-4', avatarFloatClass)}>
            {profile.avatar ? (
              <div
                className={cn('overflow-hidden ring-4 ring-white/20', getAvatarStyleClass(), getAvatarSizeClass())}
                style={getAvatarGlowStyle()}
              >
        ...
        {!profile.hideName && (
          <h1
            className={cn('font-bold mb-2 text-center break-words max-w-full', getNameSizeClass())}
            style={{
        ...
        {profile.bio && (
          <p
            className="text-center max-w-xs sm:max-w-sm mb-6 opacity-90 leading-relaxed text-sm sm:text-base break-words"
            style={{
        ...
        {profile.socialLinks.length > 0 && (
          <div className="w-full max-w-xs sm:max-w-sm space-y-3 mb-8">
                ...
                >
                  ...
                  {media.type === 'youtube' && media.url && extractYouTubeId(media.url) && (
                    <iframe
                      ...
                      className={cn('w-full', getMediaAspectClass(media.aspectRatio || 'video'))}
                      allowFullScreen
                    />
                  )}
                  ...
                  {media.caption && media.type !== 'pdf' && (
                    <p className="p-3 text-sm opacity-80 break-words" style={{ color: theme.cardTextColor }}>
                      {media.caption}
                    </p>
                  )}
                </div>
      ...
      {/* Music Player */}
      {(profile.musicUrl || profile.musicYoutubeUrl) && !isPreview && (
        <MusicPlayer
          url={profile.musicUrl}
          youtubeUrl={profile.musicYoutubeUrl}
