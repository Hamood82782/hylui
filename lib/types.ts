...
import { ArrowRight, Eye, EyeOff, Save, ExternalLink, Copy, Check, Sparkles, QrCode } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => {
                if (profile) {
                  const url = generateShareableUrl(profile, window.location.origin);
                  setShareableUrl(url);
                  setShowQRCode(true);
                }
              }}
      ...
      <div className="flex flex-col lg:flex-row min-h-[calc(100vh-57px)]">
        ...
        <div
          className={cn(
            'flex-1 border-r border-border/50 bg-muted/30 overflow-hidden',
      ...
      <div className="sm:hidden fixed bottom-4 right-4 z-50 flex flex-col gap-2">
        <Button
          variant="secondary"
          size="icon"
          onClick={() => {
